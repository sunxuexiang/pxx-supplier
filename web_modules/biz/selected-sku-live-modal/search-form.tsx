...
export default class SearchForm extends React.Component<any, any> {
  ...
  loop = (oldCateList, cateList, parentCateId) =>
    cateList
      ...
      .map((item) => {
        ...
        // );
        if (item.get('goodsCateList') && item.get('goodsCateList')?.size) {
          return (
            <TreeNode
            ...
            >
              {this.loop(item.get('goodsCateList'), item.get('goodsCateList'), item.get('cateId'))}
            </TreeNode>
          );
        }
        return (
  ...
  render() {
    ...
    const { cates, brands,wareList } = this.state;
        ...
        <Form className="filter-content" layout="inline">
          ...
          <FormItem>
            <TreeSelectGroup
              ...
              value={searchParams.cateId?searchParams.cateId.toString():'全部'}
            >
          ...
          <FormItem>
            <SelectGroup
              ...
              label="适用区域"
              dropdownStyle={{ zIndex: 1053 }}
              onChange={(val) => this.paramsOnChange('wareId', val)}
              value={searchParams.wareId.toString()}
            >
              <Option key="0" value="0">
                全部
              </Option>
              {this.props.wareList.map((v) => (
                <Option key={v.wareId} value={v.wareId + ''}>
                  {v.wareName}
                </Option>
              ))}
            </SelectGroup>
