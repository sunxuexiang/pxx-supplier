...
export default class DeliveryAddressActor extends Actor {
  ...
  init(state: IMap, res) {
    ...
    if (defaultRes.count() > 0) {
      ...
      state = state
        ...
        .set('selectedCityId', addr.get('cityId'));
  ...
  @Action('wareHouse:init')
  wareHouseInit(state: IMap, res) {
    return state.set('wareHouseList', fromJS(res));
  }

  @Action('wareHouse:select')
  wareHouseSelect(state: IMap, value) {
    return state.set('selectedWareHouse', value);
  }

  @Action('selectWareHouse:clear')
  wareHouseClear(state: IMap) {
    return state.set('selectedWareHouse', '');
  }

  @Action('deliveryWayList:init')
  deliveryWayList(state: IMap, res) {
    return state.set('deliveryWayList', fromJS(res));
  }
  @Action('addressDetail:init')
  addressDetail(state: IMap, res) {
    return state.set('addressDetail', res);
  }
}
