...
export default class ExtraForm extends React.Component<any, any> {
  ...
  render() {
    ...
    const deliverWay = this._store.state().getIn(['extra', 'deliverWay']);
    const addressDetail = this._store.state().get('addressDetail');

    const logisticsCompanyList = this._store
      .state()
      .get('logisticsCompanyList');
    ...
    const deliveryList = this._store.state().get('deliveryWayList');
    return (
      <div>
        ...
        <FormItem {...formItemLayout} label="配送方式">
          <Col span={8}>
            ...
            {deliverWay === 3 && (
              <strong style={styles.title}>自提地址：{addressDetail} </strong>
            )}
          </Col>
        </FormItem>
        {deliverWay === 1 && logisticsCompanyList && (
          <FormItem {...formItemLayout} label="物流公司选择">
            <Col span={8}>
              {getFieldDecorator('logisticsCompanyList', {
                initialValue: ' '
              })(
                <Select
                  getPopupContainer={() =>
                  ...
                  {logisticsCompanyList.map((item) => {
                    return (
                      ...
                      >
                        ...
                        <br />
                        {item.logisticsPhone ? item.logisticsPhone : '-'}
                        <br />
                        {item.logisticsAddress ? item.logisticsAddress : '-'}
                      </Option>
                    );
                  })}
                </Select>
