...
export default class AppStore extends Store {
  ...
  init = async (
    ...
    this.dispatch('goodsActor:setListLoading', true);
    ...
    this.dispatch('goodsActor:setListLoading', false);
    if (!err && res.code === Const.SUCCESS_CODE) {
      res.context.goodsPages.content.forEach((v, i) => {
        v.key = i;
      });
  ...
  onSearchGoodsForm = async (form) => {
    for (let a in form) {
      this.dispatch('form:field', { key: a, value: form[a] });
    }
    const cates: any = await getCateList();
    const brands: any = await getBrandList();
    this.transaction(() => {
  ...
  onPageSearch = async () => {
    let request: any = {
      ...
      storeGoodsSeqFlag: this.state().get('storeGoodsSeqFlag'),
      wareId:
        this.state().get('wareId') != '0' ? this.state().get('wareId') : null,
      manySpecs: this.state().get('manySpecs')
        ? this.state().get('manySpecs')
        : '0'
    ...
    this.dispatch('form:field', {
      key: 'searchForm',
      value: { ...request, ...{ goodsType: request.goodsInfoType } }
    });
    this.dispatch('goodsActor:setListLoading', true);
  ...
  // 导出表格
  exportGoodsList = async (byId?: boolean) => {
    let request: any = {
      likeGoodsName: this.state().get('likeGoodsName'),
      likeProviderName: this.state().get('likeProviderName'),
      likeGoodsInfoNo: this.state().get('likeGoodsInfoNo'),
    ...
    if (token) {
      ...
      if (byId) {
        ...
        const ids = selectedSpuKeys.toJS();
        result.goodsIds = ids;
        if (
          !result.goodsIds ||
          (result.goodsIds && result.goodsIds.length === 0)
        ) {
      ...
      console.warn(result, '参数');
      let encrypted = base64.urlEncode(result);

      // 新窗口下载
      const exportHref =
        Const.HOST + `/stockoutmanage/export/params/${encrypted}`;
  ...
  onExportModalShow = (exportModalData) => {
    this.dispatch(
      ...
      fromJS({
        ...exportModalData,
        visible: true,
        exportByParams: exportModalData.isThird
          ? this.exportGoodsList
          : this.onExportByParams,
        exportByIds: exportModalData.isThird
          ? () => this.exportGoodsList(true)
          : this.onExportByIds
      })
    );
  };
  ...
  onExportByIds = async (ids: string[]) => {
    ...
    if (ids.length === 0) {
      ...
      const goodsPageContent = this.state().getIn(['goodsPages', 'content']);
      let checkedGoods = [];
  ...
  changeAddGoodsStatus = (isLoading) => {
    console.warn('=====到这里了1------->', isLoading);
    this.dispatch('form:addGoodsLoading', isLoading);
  };
}
