...
export default class SearchForm extends React.Component<any, any> {
  ...
  componentWillReceiveProps(nextProps: any) {
    ...
    if (
      ...
      nextProps.wareId != this.props.wareId
    ) {
      this.setState({
        searchParams: {
          brandId: 0,
  ...
  render() {
    const { searchParams, wareId } = this.state;
    const { cates, brands } = this.state;
    console.log(searchParams.wareId, 'wareIdwareIdwareId优惠价wareId');
