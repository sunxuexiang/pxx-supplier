...
export default class CouponsModal extends React.Component<any, any> {
          ...
          <FormItem style={{ display: 'none' }}>
            <SelectBox>
              <SelectGroup
                ...
                // disabled={searchParams.wareId ? true : false}
                disabled
                // defaultValue={wareHouseVOPage[0].wareId}
                showSearch
                onChange={(value) => {
                  console.log(value, 'valuevaluevalue');
                  value[0] = value[0] == '0' ? -1 : value[0];
                  this._onSearchParamChange({ wareIds: value });
                }}
              >
  ...
  _onOk = () => {
    ...
    if (selectedRows.length > 10) {
      ...
      this.props.onOk(selectedRows, this.state.key);
    }
  };
    ...
    if (res.code == Const.SUCCESS_CODE) {
      ...
      couponInfos.content.forEach((coupon) => {
        wareHouseVOPage.forEach((element) => {
          if (coupon.wareId == 0 || coupon.wareId == -1) {
            coupon.wareName = '通用';
          } else if (coupon.wareId == element.wareId) {
