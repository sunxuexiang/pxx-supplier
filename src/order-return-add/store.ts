...
export default class AppStore extends Store {
   ...
   * 修改价格
   */
  editGoodssplit = (devanningId: string, value: number, bfprice) => {
    ...
    this.dispatch('tradeActor: price', { devanningId, value });
    ...
    const skuIndex = this.state()
      .get('tradeDetail')
      .get('tradeItems')
      .findIndex((item) => item.get('devanningId') == devanningId);
   ...
   * 修改价格
   */
  editGoodsPrice = (devanningId: string, value: number, bfprice) => {
    // if(Number(bfprice) < Number(value)) {
    //    this.dispatch('tradeActor: price', { skuId, bfprice });
    //    return
    // }
    ...
    this.dispatch('tradeActor: prices', { devanningId, value });
   ...
   * 修改数量
   */
  editGoodsNum = (devanningId: string, value: number) => {
    this.transaction(() => {
      // 1.修改退货商品数量
      this.dispatch('tradeActor: editGoodsNum', { devanningId, value });
      // 2.判断是否更新勾选的赠品,以及赠品数量(若修改数量的sku已被勾选,则计算并更新赠品数量)
      const skuIndex = this.state()
        .get('tradeDetail')
        .get('tradeItems')
  ...
  add = async () => {
    ...
    if (data.get('isReturn')) {
      ...
      // 如果所有商品的退货数量都为0
      console.log('1---tradeItems.size', tradeItems.size);
    ...
    let returnGifts = data.getIn(['tradeDetail', 'gifts']);
    param = param.set('returnItems', tradeItems);
    param = param.set('returnGifts', returnGifts);
    // 退款金额，退货是商品总额，退款是应付金额
    let totalPrice = data.get('isReturn')
      ? tradeItems
          .map((sku) => {
            let rowInfo = sku.toJS();
            if (!rowInfo.num) {
              return 0.0;
            }
    ...
    let totalAmortizedExpenses = tradeDetail
      .get('tradeItems')
      .map((item1) => {
        return item1
          .get('inventoryDetailSamountTrades')
          .toJS()
          .map((item) => {
            console.log('item.amortizedExpenses', item.amortizedExpenses);
            return item.amortizedExpenses;
          })
          .reduce((total, itemNum, index) => {
            return (total * 100 + itemNum * 100) / 100;
          });
      })
      .reduce((one, two) => QMFloat.accAdd(one, two));
    // 退款金额大于可退金额时
    if (
      data.get('applyStatus')
        ...
        : totalPrice > totalAmortizedExpenses
    ) {
      console.log(data.get('applyStatus'));
      console.log(data.get('applyPrice'));
      //  || data.get('canApplyPrice') > 0
      if (data.toJS().tradeDetail.activityType != '1') {
        // 在线支付要判断退款金额不能大于剩余退款金额
        console.log('isOnLine', data.get('isOnLine'));
