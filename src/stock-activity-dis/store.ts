...
//   };
  ...
  goodsInits = async ({ pageNum, pageSize } = { pageNum: 1, pageSize: 10 }) => {
    const {activityId,wareId} = this.state().toJS();
    // const query = this.state().get('form').toJS();
    this.dispatch('loading:start');
    // this.dispatch('list:inits', {content: fromJS([]) });
     let {res}=await webapi.pileActivityGoodsPage({activityId, wareId, pageNum, pageSize})
      if(res.code== Const.SUCCESS_CODE){
        this.dispatch('loading:end');
        this.dispatch('list:inits', {
          content: fromJS(res.context?.content),
          total: res.context?.total,
          pageNum: pageNum,
          size:res.context?.size
        });
      }else{
  ...
  init = async (id) => {
    ...
    this.dispatch('tab: change',wareHouseVOPage.length?wareHouseVOPage[0].wareId:null);
    this.dispatch('action: change',{key:'activityId',value:id});
    this.goodsInits();
    const { res } = await webapi.pileActivityDis(id);
    if (res.code != Const.SUCCESS_CODE) {
      message.error(res.message);
      return
    }
    ...
    // }
    this.dispatch('loading:end');
    this.dispatch('init', fromJS(res.context?.pileActivity));
  };
