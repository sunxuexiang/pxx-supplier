...
class LogisticsForm extends React.Component<any, any> {
  ...
  render() {
    ...
    return (
      <Form>
        ...
        {/* <Row type="flex" justify="start" gutter={16}>
          <Col span={8}>
            <FormItem {...formItemLayout} required label="商品规格">
              1箱
            </FormItem>
          </Col>
          <Col span={8}>
            ...
            <FormItem {...formItemLayout} label="物流体积">
              {getFieldDecorator('goodsCubage', {
                rules: [
                  ...
                  {
                    pattern:
                      /(^[1-9]([0-9]+)?(\.[0-9]{1,6})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9]{1,5})?$)/,
  ...
  _editGoods = (key: string, e) => {
    const { editGoods, setGoodsFreight, updateLogisticsForm } =
   ...
   */
  _getFreightSelect = () => {
    const { freightList, goods } = this.props.relaxProps;
    let freList = [];
    freightList.toJS().forEach((element) => {
      if (goods.get('wareId') == element.wareId) {
        freList.push(element);
      }
    });
    freList = fromJS(freList);
    return (
      <Select
        showSearch
