...
import tabList from '@/goods-detail-tab/component/tab-list';
...
export default class AppStore extends Store {
  ...
  _getGoodsDetail = async (goodsId?: string, syncStock?: boolean) => {
    ...
    this.transaction(() => {
      ...
      wareHouseVOPage.forEach((ware) => {
        if (ware.wareId == goods.get('wareId')) {
          goods = goods.set('wareName', ware.wareName);
        }
      });
      console.log(goods.toJS(), 'GOODS123');
      ...
      let images = goodsDetail.get('images').map((image, index) => {
        if (
          goodsDetail.get('relationGoodsImagesVO')?.get('imageId') ==
          image.get('imageId')
        ) {
          this.dispatch('imageActor: imgs', {
            key: 'goodsImg',
            val: image.get('artworkUrl')
          });
        }
        return Map({
          uid: index,
          resourceId: image.get('imageId'),
          name: index,
          checked:
            goodsDetail.get('relationGoodsImagesVO')?.get('imageId') ==
            image.get('imageId')
      ...
      let goodsImageStypeVOSList =
        goodsDetail.get('goodsImageStypeVOS').toJS() || [];
      let salesPromotionImages = goodsImageStypeVOSList
        .filter((item) => item.type == 0)
        .map((item, index) => {
          if (
            goodsDetail.get('relationGoodsImagesVO')?.get('cxImageId') ==
            item.imagesTypeId
          ) {
            this.dispatch('imageActor: imgs', {
              key: 'salesPromotionImg',
              val: item.artwork_url
            });
          }
          return {
            uid: index,
            ...
            checked:
              ...
              item.imagesTypeId
                ? true
                : false,
            resourceId: item.imagesTypeId,
            goodsId: item.goodsId,
            artworkUrl: item.artwork_url
          };
        });
      let synthesisImages = goodsImageStypeVOSList
      ...
      this.dispatch(
        'salesPromotionImageActor: editImages',
        salesPromotionImages
      );
      this.dispatch('imageActor: imgs', {
        key: 'synthesisImages',
        val: fromJS(synthesisImages)
      });
      let videoObj = Map({
        uid: 0,
        size: 1,
  ...
  //选择要合成的图片
  onCheckChange = (tag, checked, imgName?) => {
    let images = this.state()
      .get(imgName ? imgName : 'images')
      .toJS();
    console.log(images, 'imagesimages');
    images = images.map((item) => {
      item.checked = item.uid == tag.uid ? checked : false;
      return item;
    });
    let img = images.filter((item) => item.checked)[0]?.artworkUrl || '';
    if (imgName == 'salesPromotionImages') {
      this.dispatch('salesPromotionImageActor: editImages', images);
      this.dispatch('imageActor: imgs', {
        key: 'salesPromotionImg',
        val: img
      });
      ...
      this.dispatch('imageActor: imgs', { key: 'goodsImg', val: img });
    }
  };

  onImgChange = (key, value) => {
    this.dispatch('imageActor: imgs', { key: key, val: value });
  };
  //删除合成图片
  onDelSynthesisImages = () => {
    let { salesPromotionImages, images } = this.state().toJS();
    let t = salesPromotionImages.map((item) => {
      return {
        ...item,
        checked: false
      };
    ...
    console.log(salesPromotionImages, 'salesPromotionImages');
    this.dispatch('imageActor: editImages', [...images]);
    this.dispatch('salesPromotionImageActor: editImages', t);
    this.onImgChange('synthesisImages', fromJS([]));
    this.dispatch('imageActor: imgs', { key: 'goodsImg', val: '' });
    this.dispatch('imageActor: imgs', { key: 'salesPromotionImg', val: '' });
  };
  /**
   * 修改促销图片
   */
  salesPromotionEditImages = (images: IList) => {
  ...
  saveMain = async () => {
    ...
    const images = data.get('images').map((item) => {
      return {
        ...
        imageId: item.get('resourceKey') ? null : item.get('resourceId'),
        checkFlag: item.get('checked') ? 1 : 0
      };
    });
    let goodsImg = this.state().get('goodsImg');
    let checkImageId = null;
    data.get('images').forEach((item) => {
      // if(item.get('checked')){
      //   if(item.get('artworkUrl')!=goodsImg){
      //     return message.error('你的合成图片和选择图片不一致')
      //   }
      // }
      if (item.get('checked') && item.get('imageId')) {
        checkImageId = item.get('resourceId');
      }
    });
    if (images.toJS().length <= 0) {
      message.error('必须上传一张商品图片');
      return;
    }
    param = param.set('checkImageId', checkImageId);
    param = param.set('images', images);
    let salesPromotionImages = this.state()
      .get('salesPromotionImages')
      .toJS()
      .map((item) =>
        Map({
          artwork_url: item.artworkUrl,
          goodsId: data.get('goods').get('goodsId'),
          // resourceId:item.resourceId,
          type: 0,
          checkFlag: item.checked ? 1 : 0
        })
      );

    let synthesisImagesList = this.state()
      .get('synthesisImages')
      .toJS()
      .map((item) =>
        Map({
          artwork_url: item.url,
          goodsId: data.get('goods').get('goodsId'),
          // resourceId:item.resourceId,
          type: 1
        })
      );
    const goodsImageVOS = [...salesPromotionImages, ...synthesisImagesList];
    param = param.set('goodsImageVOS', goodsImageVOS);
    // -----商品规格列表-------
      ...
      goodsList = goodsList.push(
        Map({
          ...
          goodsWareStocks: item.get('goodsWareStocks')
            ...
            : null,
          presellStock: item.get('presellStock')
            ? Number(item.get('presellStock'))
            : ''
        })
      );
    });
  ...
  onCanvasModal = async (val, blob) => {
    if (val == 5) {
      let file2 = new File([blob], 'a.jpg', { type: 'image/jpg' });
      let formDatas = new FormData();
      formDatas.append('uploadFile', file2);
      ...
      // let res=await exportMarketing(formDatas);
      ...
      xhr.onreadystatechange = () => {
        if (xhr.readyState === 4 && xhr.status === 200) {
          let list = [
            {
              url: xhr.responseText.slice(2, xhr.responseText.length - 2),
              uid: 0,
              name: 'xxx.jpg',
              status: 'done'
            }
          ...
          this.dispatch('canvasActor: canvasModalVisible', false);
        }
      };
    } else {
      this.dispatch('canvasActor: canvasModalVisible', val);
    }
  };
  ...
  modalVisible = async (
    ...
    this.dispatch('modal: visible', { imgType, skuId, imgName });
  };
  ...
  beSureImages = () => {
    ...
    const imgName = this.state().get('imgName');
    if (imgType === 0) {
      let images = this.state().get(imgName ? imgName : 'images');
      images = images.concat(chooseImgs).toJS();
      images.forEach((item, index) => {
        item.uid = index;
      });
      console.log(chooseImgs, 'images');
      if (imgName == 'salesPromotionImages') {
        this.dispatch('salesPromotionImageActor: editImages', images);
      } else {
        this.dispatch('imageActor: editImages', images);
   ...
   * @param id
   */
  salesPromotionRemoveImg = ({ type, id }) => {
    if (type === 0) {
      this.dispatch('salesPromotionImageActor: remove', id);
    } else {
      this.dispatch('goodsSpecActor: removeImg', id);
    }
  };
