...
export default class settingForm extends React.Component<any, any> {
  ...
  constructor(props, ctx) {
    ...
    this.state = {
      fileList: [{ status: 'done', videoId: 'default' }]
    };
  }
          ...
          <Col span={18}>
            ...
            >
              <div className="clearfix logoImg" style={{ paddingTop: 5 }}>
                <div>
                  {/* {console.warn(this.state.fileList,videoId)} */}
                      ...
                      <div className="ant-upload-list-item ant-upload-list-item-done">
                        ...
                        <span className="ant-upload-list-item-actions">
                          <i
                            ...
                            onClick={() => this._videoDetail(artworkUrl)}
                          >
                            <Icon type="eye" />
                          </i>
                          <i
                            title="Remove file"
                            onClick={() => this._removeVideo()}
  ...
  _setFileList = (fileList) => {
    ...
    const artworkUrlName =
      fileList[0] && fileList[0].response && fileList[0].response[0];
    this._store.paramFormChange(artworkUrlName);
  };

  _handleSubmit = () => {
    let artworkUrl = this._store
      .state()
      .get('settings')
      .get('resourceKey');
    if (this.state.fileList.length == 0) {
      ...
      if (this.state.fileList[0].videoId != 'default') {
        artworkUrl =
          this.state.fileList &&
          this.state.fileList[0] &&
    ...
    (this._store as any).addVideo(artworkUrl);
  };
}
