...
export default class DiscountLevels extends React.Component<any, any> {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.isOverlap && nextProps.fullDiscountLevelList.length > 1) {
      this.setState(
        {
          fullDiscountLevelList: nextProps.fullDiscountLevelList.slice(0, 1)
        },
        () => {
          const { onChangeBack } = this.props;
          onChangeBack(nextProps.fullDiscountLevelList.slice(0, 1));
        }
      );
    }
  }
  ...
  render() {
    const { isFullCount, fullDiscountLevelList, isOverlap } = this.state;
   ...
   */
  addLevels = () => {
    const { fullDiscountLevelList, isOverlap } = this.state;
