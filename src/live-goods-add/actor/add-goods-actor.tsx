...
export default class AddGoodsActor extends Actor {
  defaultState() {
    return {
      // 数据总条数
      total: 0,
   ...
   * 修改选择的单品的字段
   */
  @Action('change:sku:info')
  changeSelectSkuInfo(state, { skuId, key, value }) {
    return state.update('goodsRows', (skus) => {
      return skus.map((sku) => {
   ...
   * 编辑价格信息
   */
  @Action('change:goodsRowsPrice')
  changeGoodsRowsPrice(state: IMap, params) {
    return state.set('goodsRowsPrice', params);
  }

  /**
   * 设置已添加商品列表
   */
  @Action('info:setAllGoodsList')
  setAllGoodsList(state: IMap, params) {
    return state.set('allGoodsList', params);
  }
}
