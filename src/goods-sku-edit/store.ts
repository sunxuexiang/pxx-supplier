...
export default class AppStore extends Store {
  ...
  _getGoodsDetail = async (goodsId?: string, syncStock?: boolean) => {
    ...
    if (goodRes.res.code == Const.SUCCESS_CODE) {
      ...
      //   '11111111111'
      // );
      goodRes.res.context.goodsInfo.choseProductGoodsInfo =
        goodRes.res.context.choseProductGoodsInfo;
      const conetet = goodRes.res.context;
      let subtitleList = [];
      const suit = conetet.goods.goodsSubtitle.substr(
        conetet.goods.goodsSubtitle.length - 1,
        1
      ); //单位
      // console.log(conetet.goodsInfo.addStep / 2, 'conetet.goodsInfo.addStep / 2');
      let subPindt = conetet.goodsInfo.addStep / 2;
      let subPindt4 = conetet.goodsInfo.addStep / 4;
      if (subPindt % 1 === 0) {
        let sub1 = '0.5' + conetet.goods.goodsUnit + '=' + subPindt + suit;
        subtitleList.push({
          key: '0',
          value: sub1
        });
      ...
      //   let sub2 =
      //     '0.25' +
      //     conetet.goods.goodsUnit +
      //     '=' +
      //     conetet.goodsInfo.addStep / 4 +
      //     suit;
      //   subtitleList.push({
      //     key: '1',
      //     value: sub2
      ...
      conetet.goods.devanningType = conetet.goodsInfo.devanningType;
      conetet.goodsInfo.isDevanning = conetet.goodsInfo.devanningType ? 1 : 0;
      // console.log(subtitleList, 'subtitleListsubtitleList');
      // conetet.goods.isDevanninglisngsxx = subtitleList.length > 0 ? 1 : 0;
      conetet.goodsInfo.subtitleList =
        subtitleList.length > 0 ? subtitleList : null;
      // console.log(conetet, 'goodRes.res.contextgoodRes.res.context');

      // this.dispatch('goodsActor: editGoodsrAssignArea', goodRes.res.context.choseProductGoodsInfo);
      goodsDetail = fromJS(conetet);
      let skuWareStocks;
      //新查询的同步信息
      if (syncStock) {
        let param = Map();
      ...
      this.dispatch(
        'goodsActor: editGoodsrAssignArea',
        goodsDetail.get('goodsInfo')
      );
      ...
      this.transaction(() => {
        ...
        console.log(
          goodsDetail.get('goodsInfo').toJS(),
          'goodsInfogoodsInfogoodsInfogoodsInfogoodsInfo'
        );
        ...
        let goodsList = List();
        let suitGoods = List();
        if (conetet.devanningGoodsInfoVOS) {
          conetet.devanningGoodsInfoVOS.forEach((element) => {
            if (element.addStep != goodsInfo.get('addStep')) {
              suitGoods = suitGoods.push(element);
            }
          });
        }
  ...
  edtailsuitGoods = ({ keyas, value, id }) => {
    ...
    // console.log(goods.toJS(), 'goods123123');
    // id, key, value
    // console.log(keyas, value, id, 'key, e, id');

    this.dispatch('suitGoods: fild', {
      id,
      key: keyas,
      value
    });
  ...
  syncErpStock = async (erpGoodsInfoNo, goodsInfoId) => {
    const goodsWareStocks = this.state()
      .get('goodsList')
      .get(0)
      .get('goodsWareStocks')
      .get(0)
    ...
    await syncErpGoodsWareStock({
      ...
      wareId: goodsWareStocks.wareId ? goodsWareStocks.wareId : null
    }).then(({ res, err }) => {
      ...
      if (res.code == Const.SUCCESS_CODE) {
        ...
        let suitGoods = this.state().get('suitGoods');
        let wareItem = [];
        goodsWareStockVOS.forEach((vos) => {
          if (vos.wareId == goodsWareStocks.wareId) {
            wareItem.push(vos);
          }
        });
        if (goodsList) {
          let goodsInfoList = goodsList.map((g) => {
            if (g.get('goodsInfoId') == goodsInfoId) {
              g = g.set('goodsWareStocks', wareItem);
            }
            return g;
          });
  ...
  saveMain = async () => {
    ...
    const { areaParam, cityAreaParam, singleOrderAssignAreaParam } =
      this.state().toJS();
    ...
    let suitGoodsa = data.get('suitGoods').toJS();
      console.log(data.toJS(),'goodsInfogoodsInfo');
    ...
    if (
      goodsInfo.get('isSuitGoods') == 1 &&
      data.get('choseProductSkuId').length == 1
    ) {
      goodsInfo = goodsInfo.set(
        'choseProductSkuId',
    ...
    if (allowedPurchaseAreaNames == '传空') {
      goodsInfo = goodsInfo.set('singleOrderAssignAreaName', '');
      // 单笔限购区
      goodsInfo = goodsInfo.set('singleOrderAssignArea', '');
    } else {
    ...
    if (allowedPurchaseAreaName == '传空') {
      goodsInfo = goodsInfo.set('allowedPurchaseAreaName', '');
      // 销售区
      goodsInfo = goodsInfo.set('allowedPurchaseArea', '');
    } else {
    ...
    goodsInfo = goodsInfo.set(
      ...
      data.getIn(['goods', 'singleOrderPurchaseNum'])
    );

    // if (singleOrderAssignAreaParam.destinationArea.length > 0) {
    //   if (data.getIn(['goods', 'singleOrderPurchaseNum']) <= 0) {
    //     message.error('请填写大于0的值')
    //     return
    //   }
    //   else if (!data.getIn(['goods', 'singleOrderPurchaseNum']) || data.getIn(['goods', 'singleOrderPurchaseNum']) == ' ') {
    //     message.error('请填写单笔订单限购数量')
    //     return
    //   }
    // }
    // if(data.getIn(['goods', 'singleOrderPurchaseNum']) <= 0 || data.getIn(['goods', 'singleOrderPurchaseNum']) != '') {
    //   message.error('请填写单笔订单限购数量大于0')
    //     return
    // }
    ...
    if (
      singleOrderAssignAreaParam.destinationArea.length == 0 &&
      !data.getIn(['goods', 'singleOrderAssignArea'])
    ) {
      ...
      if (
        singleOrderAssignAreaParam.destinationArea.length > 0 &&
        data.getIn(['goods', 'singleOrderPurchaseNum']) <= 0
      ) {
        message.error('请填写大于0的值');
        return;
      } else if (
    ...
    // console.log(
    //   goodsInfo.toJS(),
    //   'goodsInfogoodsInfogoodsInfogoodsInfo',
    //   data.get('goods').toJS()
    // );
    // 起售功能
    // if (data.getIn(['goods', 'isStartBuyNum']) == 1) {
    //   goodsInfo = goodsInfo.set('isStartBuyNum', data.getIn(['goods', 'isStartBuyNum']));
    //   goodsInfo = goodsInfo.set('startBuyNum', data.getIn(['goods', 'startBuyNum']) ? data.getIn(['goods', 'startBuyNum']) : 2);
    // }
    // 拆箱
    goodsInfo = goodsInfo.set(
      'devanningType',
      data.getIn(['goods', 'devanningType'])
    );
    // console.log(data.getIn(['goods', 'isDevanning']),'isDevanning');
    // console.log(data.getIn(['goods', 'devanningType']),'devanningType');
    if (
      data.getIn(['goods', 'isDevanning']) == 1 &&
      data.getIn(['goods', 'devanningType']) == null
    ) {
      message.error('请选择拆箱规格');
      return;
    }
    goodsInfo = goodsInfo.set(
      'isDevanning',
      data.getIn(['goods', 'isDevanning'])
        ? data.getIn(['goods', 'isDevanning'])
        : 0
    );
    goodsInfo = goodsInfo.set(
      'isSurprisePrice',
      data.getIn(['goods', 'isSurprisePrice'])
        ? data.getIn(['goods', 'isSurprisePrice'])
        : 0
    );
    // return
    ...
    if(!goodsInfo.get('singleOrderAssignArea')){
      goodsInfo=goodsInfo.set('singleOrderPurchaseNum','')
    };
    param = param.set('devanningGoodsInfos', [...suitGoodsa]);
