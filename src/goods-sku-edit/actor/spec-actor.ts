...
export default class GoodsSpecActor extends Actor {
  defaultState() {
    return {
      ...
      suitGoods: [{ id: this._getRandom(), index: 1 }],
  ...
  init(
    state,
    {
      ...
      goodsList,
      suitGoods
    }: { goodsSpecs: IList; goodsList: IList; suitGoods: IList }
  ) {
    if (!goodsSpecs.isEmpty()) {
      state = state.set('goodsSpecs', goodsSpecs);
    }
    return state.set('goodsList', goodsList).set('suitGoods', suitGoods);
  }
  @Action('suitGoods: fild')
  edtailsuitGoods(
    state,
    { id, key, value }: { id: string; key: string; value: any }
  ) {
    // console.log(state.toJS().suitGoods, '22222');
    let suitList = state.toJS().suitGoods;
    suitList.forEach((el) => {
      if (el.devanningId == id) {
        el[value] = key;
      }
    });
    // console.log(suitList, '1111111');
    return state.set('suitGoods', fromJS(suitList));
    // return state.update('suitGoods', (suitGoods) => {
    //   console.log(suitGoods.toJS(),'suitGoods');

    //   const index = suitGoods.findIndex((item) => item.devanningId == id);
    //   console.log(index,'indexindex');
    //   // suitGoods[index][key] = value
    //   return suitGoods.update(index, (item) => item.set(key, value));
    // });
  }
