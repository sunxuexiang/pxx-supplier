...
export default class AppStore extends Store {
  ...
  apply = async (tid) => {
    ...
    let errMsg;
    let canApply = false;
    let canReturnNumList = []
    if(res.context.tradeItems.length > 0) {
      res.context.tradeItems.forEach(element => {
        if(element.canReturnNum && element.canReturnNum > 0) {
          canReturnNumList.push(element)
        }
      });
    }
    if(canReturnNumList.length <= 0) {
      message.error('无可退商品');
      return
    }
    console.log(res.context.tradeItems,'canReturnNum')
