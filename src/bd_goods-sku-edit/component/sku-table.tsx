...
class SkuForm extends React.Component<any, any> {
  ...
  render() {
    ...
    const { goodsList, goods, doInitGoodList, goodsSpecs, singleOrderAssignAreaList,wareHousesList, singleOrderAssignAreaParam } =
      ...
      <div>
        ...
        <Form style={{ marginTop: 15, marginBottom: 20, overflowX: 'scroll' }}>
          <Row type="flex" justify="start">
            <Col span={14}>
              <FormItem {...formItemLayout} label="以下设置同步其他仓">
                  {getFieldDecorator('wareIds', {
                    onChange: (e)=>{
                      this._editGoods('wareIds',e);
                    },
                    initialValue: goods.get('wareIds')
                  })(
                    <Checkbox.Group style={{ width: '100%' }} >
                        {
                          wareHousesList.toJS().map(item=><Checkbox disabled={item.wareId==goods.get('wareId')?true:false} value={item.wareId}>{item.wareName}</Checkbox>)
                        }
                    </Checkbox.Group>
                  )}
                </FormItem>
          ...
          <Row type="flex" justify="start">
            <Col span={8}>
              <FormItem {...formItemLayout} label="基础单位/售价">
                  <div>
                  <Button disabled>{goods.get('goodsSubtitle')}</Button>
                    {/* <Button disabled>1{goods.get('goodsUnit')}/{goodsList.size&&goods.get('addStep')?(goodsList.toJS()[0].vipPrice/Number(goods.get('addStep'))).toFixed(2):0}元</Button> */}
                  </div>
              </FormItem>
            </Col>
          </Row>
          <Row type="flex" justify="start">
            ...
            <Col span={8}>
              <FormItem {...formItemLayout} label="整箱规格：">
                {getFieldDecorator('fullStep', {
                  rules: [
                    {
                      required: true,
                      message: '请输入整箱规格'
                    },
                    {
                      pattern:/^[0-9]*[1-9][0-9]*$/,
                      // type:'number',
                      transform:(val)=>{return val?Number(val):0},
                      message: '只能是整数'
                    }
                  ],
                  onChange: this._editQueryPrice.bind(this, 'fullStep'),
                  initialValue: goods.get('fullStep')?String(goods.get('fullStep')):''
                })(
                  <div>
                    1箱=&nbsp;
                    <Input  style={{width:'100px'}} value={goods.get('fullStep')?String(goods.get('fullStep')):''}  />
                    &nbsp;{goods.get('goodsUnit')}
                  </div>
                )}
              </FormItem>
          ...
          <Row type="flex" justify="start">
            <Col span={8}>
              <FormItem {...formItemLayout} label="设置散批规格：">
                {getFieldDecorator('addStep', {
                  rules: [
                    {
                      required: true,
                      message: '请设置散批规格'
                  ...
                  onChange:(e)=>{
                    this._editQueryPrice('addStep',e);
                    let t=Number(e.target.value||0);
                    // let r=Number(goods.get('goodsSubtitleNew').split('元')[0]);
                    let r=goods.get('erpPrice')||0;
                    let num=(r*t).toFixed(2);
                  
                    let obj={};
                    goodsList.toJS().forEach(ele => {
                      obj['marketPrice_'+ele.id]=num;
                      obj['vipPrice_' + ele.id]=num;
                    });
                    this.props.form.setFieldsValue(obj);
                    this._editGoods('marketPrice',{target:{value:num}});
                    this._editGoods('vipPrice',{target:{value:num}});
                    // this._editQueryPrice.bind(this, 'addStep')
                  },
                  initialValue:goods.get('addStep')?String(goods.get('addStep')):''
                })(
                  <div>
                    1份=&nbsp;
                    <Input style={{width:'100px'}}  value={goods.get('addStep')?String(goods.get('addStep')):''} />
                    &nbsp;{goods.get('goodsUnit')}
                  </div>
                )}
              </FormItem>
 ...
 */
  ...
  _getColumns = () => {
          ...
          <Col>
            <FormItem>
              {getFieldDecorator('marketPrice_' + rowInfo.id, {
                ...
                onChange: (e)=>{
                  this._editGoods('marketPrice',e);
                  this._editGoods('vipPrice',e);
                  let obj={};
                  obj['vipPrice_' + rowInfo.id]=e.target.value;
                  this.props.form.setFieldsValue(obj);
                  this.props.form.validateFields(['retailPrice_' + rowInfo.id],(errors, values)=>{
                    console.log(values)
                  })
                },
    ...
    columns = columns.push({
      ...
      key: 'retailPrice',
      width: '9%',
      render: (rowInfo,t,index) => (
        <Row>
          <Col>
            <FormItem>
              {getFieldDecorator('retailPrice_' + rowInfo.id, {
                rules: [
                  // {
                  //   required: true,
                  //   message: '请填写划线价'
                  ...
                  // },
                  {
                    validator:(rule, value,callback) => {
                      let marketPrice=this.props.form.getFieldValue('marketPrice_' + rowInfo.id)
                      let vipPrice=this.props.form.getFieldValue('vipPrice_' + rowInfo.id)
                      if((value<=marketPrice||value<=vipPrice)&&value!=''&&value!=null){
                        callback('不能小于等于门店价和大客户价');
                        return
                      }
                      callback();
                    }
                ...
                onChange: this._editGoods.bind(this, 'retailPrice'),
                initialValue: rowInfo.retailPrice
              })(<Input  />)}
            </FormItem>
          </Col>
        </Row>
                    ...
                    <FormItem>
                      ...
                      {getFieldDecorator(
                        'goodsWareStocks_' + item.get('id'),
                        {
                          ...
                          onChange:(e)=>{
                            this._editGoodsStockWare(rowInfo.id,item.get('id'),'stock',e)
                          },
                        ...
                        <div>
                          ...
                          <Input value={item.get('stock')}  style={styles.wareStock} />
                        </div>
                      )}
                    </FormItem>
                  );
