...
export default class AppStore extends Store {
  ...
  _getGoodsDetail = async (goodsId?: string, syncStock?: boolean) => {
    ...
    if (goodRes.res.code == Const.SUCCESS_CODE) {
      ...
      this.transaction(() => {
        ...
        goodsInfo=goodsInfo.set('goodsSubtitle',goods.get('goodsSubtitle'));
        goodsInfo=goodsInfo.set('goodsSubtitleNew',goods.get('goodsSubtitleNew'));
        goodsInfo=goodsInfo.set('wareIds',[goods.get('wareId')] );
        // goodsInfo=goodsInfo.set('fullStep', goodsInfo.get('fullStep'));
        // goodsInfo=goodsInfo.set('addStep',goodsInfo.get('addStep'));
        ...
        this.dispatch('wareHousesList',fromJS(goodsDetail.get('wareHousesList').toJS()));
    ...
    await syncErpGoodsWareStock({
      ...
      if (res.code == Const.SUCCESS_CODE) {
        ...
        if (goodsList) {
          ...
          console.log(goodsInfoList,'goodsInfoListgoodsInfoList')
  ...
  saveMain = async () => {
    ...
    goodsInfo = goodsInfo.set('fullStep', data.getIn(['goods', 'fullStep']));
    goodsInfo = goodsInfo.set('addStep', data.getIn(['goods', 'addStep']));
    goodsInfo = goodsInfo.set('retailPrice', data.getIn(['goods', 'retailPrice']));
    ...
    param = param.set(
      'isSynchronization', data.getIn(['goods', 'wareIds']).length>1?true:false
    );
    param = param.set(
      'wareIds', data.getIn(['goods', 'wareIds'])
    );
    //保存参数
    // console.info('--param--');
