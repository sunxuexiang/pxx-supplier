...
const importGoods = (ext) => {
  ...
  let encrypted = base64.urlEncode(ext);
  return Fetch('/goods/storeImport/' + encrypted, {
    method: 'GET'
  });
};

export default class GoodsAddImport extends React.Component<any, any> {
  constructor(props) {
    super(props);
    this.state = {
      current: 0,
  ...
  render() {
    ...
    return (
      <div>
        <BreadCrumb>
          <Breadcrumb.Item>商品新增导入</Breadcrumb.Item>
        </BreadCrumb>
          ...
          <div>
            <Alert
              ...
              description={
                <ul>
                  <li>
                    1、请先下载商品新增导入模板，并按照批注中的要求填写商品新增数据，未按要求填写将会导致导入失败。
                  </li>
                  <li>
                    2、请选择 .xlsx或
                    .xls文件，文件大小≤2M，每次只能导入一个文件，建议每次导入不超过500条商品新增数据。
                  </li>
                </ul>
              }
            />
              ...
              <Spin spinning={this.state.loading}>
                <div className="steps-content" style={styles.center}>
                  <Dragger
                    ...
                    accept=".xls,.xlsx"
                    headers={header}
                    action={Const.HOST + '/goods/goodsAdd/excel/upload'}
  ...
  _init = () => {
    ...
    let errBtn = false;
    let errUrl = '';
    this.setState({ err, ext, fileName, loading, isImport, errBtn, errUrl });
    this.prev();
  };
  ...
  toDownTempl() {
    ...
    if (token) {
      ...
      // 新窗口下载
      const exportHref =
        Const.HOST + `/goods/goodsAdd/storeExcel/template/${encrypted}`;
  ...
  _importGoods = async () => {
    ...
    if (importRes.res.code == 'K-030404') {
      ...
      let err = true;
      let errBtn = true;
      this.setState({ loading, err, errBtn, errUrl: importRes.res.message });
