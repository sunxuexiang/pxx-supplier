...
const importGoods = (ext) => {
  return Fetch('/goods/goodsModify/import/' + ext, {
    method: 'GET'
  });
};
...
export default class GoodsModifyImport extends React.Component<any, any> {
          ...
          <div>
            <Alert
              ...
              description={
                <ul>
                  <li>
                    1、请先下载商品修改导入模板，并按照批注中的要求填写商品修改数据，未按要求填写将会导致导入失败。
                  </li>
                  <li>
                    2、请选择 .xlsx或
              ...
              <Spin spinning={this.state.loading}>
                <div className="steps-content" style={styles.center}>
                  <Dragger
                    ...
                    accept=".xls,.xlsx"
                    headers={header}
                    action={Const.HOST + '/goods/goodsModify/excel/upload'}
  ...
  toDownTempl() {
    ...
    if (token) {
      ...
      // 新窗口下载
      const exportHref =
        Const.HOST + `/goods/goodsModify/excel/template/${encrypted}`;
  ...
  toExcel = () => {
    ...
    if (atoken != '') {
      ...
      // 新窗口下载
      const exportHref =
        Const.HOST + `/goods/goodsModify/excel/err/${ext}/${encrypted}`;
      window.open(exportHref);
    } else {
      message.error('请登录');
    }
