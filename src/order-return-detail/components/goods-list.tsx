...
import { Const, Logistics,history } from 'qmkit';
...
const columns = [
  ...
  {
    title: '退货金额小计',
    render: (ruif) => <div>￥{(ruif.price * ruif.num).toFixed(2)}</div>
  },
  {
    ...
    title: '商品来源(囤货单)',
    render: (text,row) =>{
      let obj=row.returnGoodsList?row.returnGoodsList[0]:{newPileOrderNo:''}
      return <a href="javascript:void(0);" onClick={() => {
        history.push({ pathname: `/th_order-detail/${obj.newPileOrderNo}`})
      }}>{obj.newPileOrderNo}</a>
    }
  }
];
...
export default class GoodsList extends React.Component<any, any> {
  ...
  render() {
    ...
    images.forEach((imageString) => {
      if(imageString) {
        imageList.push(JSON.parse(imageString))
      }
    });
    console.log(images);
        ...
        <div style={styles.detailBox as any}>
          ...
          <div style={styles.priceBox}>
            ...
            <label style={styles.priceItem as any}>
              ...
              <strong>{detail.getIn(['returnPrice', 'balanceReturnPrice'])}</strong>
            </label>
            {refundStatus === 2 && (
              <label style={styles.priceItem as any}>
            ...
            {refundStatus === 2 && (
              <label style={styles.priceItem as any}>
                ...
                <strong>{detail.getIn(['returnPrice', 'actualBalanceReturnPrice'])}</strong>
              </label>
            )}
            {refundStatus === 2 && (
              <label style={styles.priceItem as any}>
