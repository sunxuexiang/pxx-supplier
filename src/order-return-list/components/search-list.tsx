...
import { AuthWrapper, Const, noop, util, history } from 'qmkit';
...
export default class SearchList extends React.Component<any, any> {
                ...
                >
                  <thead className="ant-table-thead">
                    <tr>
                      ...
                      <th style={{ width: '6%' }}>应退金额</th>
                      <th style={{ width: '6%' }}>应退积分</th>
                      <th style={{ width: '6%' }}>实退金额</th>
                      <th style={{ width: '6%' }}>实退积分</th>
                      <th style={{ width: '7%' }}>发货仓</th>
  ...
  _renderContent(dataList) {
    ...
    return dataList.map((v, index) => {
      ...
      console.log(returnFlowState, returnType, util.isThirdStore(), '789');
      return (
        <tr
            ...
            >
                    ...
                    >
                      ...
                      <span style={{ marginRight: 0, float: 'right' }}>
                        ...
                        {returnFlowState === 'DELIVERED' &&
                          returnType == 'RETURN' &&
                          util.isThirdStore() && (
                            <AuthWrapper functionName="f_receive_confirm">
                              <Popconfirm
                                title="若您已收到客户的退货商品，确认后将进入退款审核环节"
                                onConfirm={() => this._receiveConfirm(rid)}
                                okText="是"
                                cancelText="否"
                              >
                                <a
                        ...
                        {/*{!isProvider && returnFlowState === 'DELIVERED' && (*/}
              ...
              <tbody>
                <tr>
                  ...
                  <td style={{ width: '8%' }}>
                    {/* 收件人姓名 */}
                    {/* {v.get('buyer') ?  v.getIn(['buyer', 'name']).substr(0,3) + "****" + v.getIn(['buyer', 'name']).substr(7) : ''}  */}
                  ...
                  {/*应退积分*/}
                  <td style={{ width: '6%' }}>{applyPoints}</td>
                  {/*实退金额*/}
                  <td style={{ width: '6%' }}>
                  ...
                  {/*发货仓*/}
                  <td style={{ width: '7%' }}>{v.get('wareName') || '-'}</td>
                  {/*订单类型*/}
                  <td style={{ width: '8%' }}>
  ...
  // 鲸币退款
  _jingbiRefund(record) {
    if (record) {
      if (
        record?.returnPrice?.totalPrice &&
        record?.returnPrice?.totalPrice > 0
      ) {
        history.push({
          pathname: '/jinbi-account',
          state: {
            isOrderReturn: true,
            currentData: {
              customerAccount: record?.buyer?.account || '',
              balance: record?.returnPrice?.totalPrice || '',
              relationOrderId: record?.tid || '',
              returnOrderCode: record?.id || '',
              remark: record?.description || ''
            }
          }
        });
      } else {
        message.error('应退金额必须大于0！');
      }
    }
  }
  ...
  //第三方商家确认收货
  _receiveConfirm = (rid: string) => {
    this.props.relaxProps.receiveConfirm(rid);
  };
